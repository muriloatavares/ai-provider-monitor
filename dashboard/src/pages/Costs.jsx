...
export default function Costs() {
  ...
  if (loading) return <div className="text-[#66fcf1] animate-pulse">Loading cost data...</div>

  if (!costs || !costs.grandTotalCost) {
    return (
      <div className="card text-center py-12">
        <DollarSign className="w-12 h-12 mx-auto text-gray-600 mb-4" />
        <p className="text-gray-400">No cost data available. Run the pricing engine and benchmarks first.</p>
      </div>
    )
  }
  ...
  const avgCostReq = sessions.length > 0 ? (costs.grandTotalCost / sessions.length) : 0;
  
  // Highest & Lowest consumer
  const sortedSessions = [...sessions].sort((a, b) => b.exactCost - a.exactCost);
  const highestConsumer = sortedSessions[0] || null;
  const lowestConsumer = [...sortedSessions].filter(s => s.exactCost > 0).pop() || null;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 border-b border-[#45a29e]/20 pb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Financial tracking and API spend</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <CostCard 
          ...
          value={highestConsumer ? highestConsumer.model.split('/').pop() : 'N/A'}
          icon={TrendingUp}
          subtitle={highestConsumer ? `$${highestConsumer.exactCost.toFixed(6)}` : ''}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <div className="card overflow-x-auto p-0">
          <div className="p-4 border-b border-[#45a29e]/20 bg-[#1f2833]/50">
            <h3 className="font-bold text-gray-100">Cost Per Request (Session Log)</h3>
          </div>
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-[#45a29e]/10 max-h-96 overflow-y-auto">
              {sessions.map((s, idx) => (
                <tr key={idx} className="hover:bg-[#45a29e]/5 transition-colors">
        ...
        <div className="card flex items-center justify-center border-dashed border-[#45a29e]/20">
          <div className="text-center text-gray-500">
            ...
            <p className="text-xs mt-2">Requires Historical Database Integration (V2 Phase 11)</p>
          </div>
        </div>
      </div>
    </div>
