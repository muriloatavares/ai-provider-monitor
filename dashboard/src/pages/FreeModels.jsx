...
export default function FreeModels() {
  ...
  if (loading) return <div className="text-[#66fcf1] animate-pulse">Loading free models discovery...</div>

  if (!freeData || !freeData.models) {
    return (
      <div className="card text-center py-12">
        <Gift className="w-12 h-12 mx-auto text-gray-600 mb-4" />
        <p className="text-gray-400">No free models data available. Run 'npm run models:free' first.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-[#45a29e]/20 pb-4 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-100">Free Models Discovery</h2>
            <p className="text-sm text-gray-400">100% Free LLMs found on OpenRouter</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-[#1f2833] p-1 rounded-lg border border-[#45a29e]/20">
          ...
          >
            ...
            <option value="online">Online ({models.filter(m => m.status === 'online').length})</option>
            <option value="offline">Offline ({models.filter(m => m.status === 'offline').length})</option>
          </select>
        </div>
      </div>
        ...
        <table className="w-full text-left text-sm text-gray-300">
          ...
          <tbody className="divide-y divide-[#45a29e]/10">
            {filteredModels.length === 0 ? (
              ...
              <tr key={m.model} className={`hover:bg-[#45a29e]/5 transition-colors ${m.status === 'offline' ? 'opacity-60' : ''}`}>
                ...
                <td className="px-6 py-4 text-xs text-red-400 max-w-[200px] truncate" title={m.error || ''}>
                  {m.error || '-'}
                </td>
              </tr>
            ))}
