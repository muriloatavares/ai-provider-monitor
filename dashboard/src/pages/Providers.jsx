...
export default function Providers() {
  ...
  if (loading) return <div className="text-[#66fcf1] animate-pulse">Loading providers data...</div>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 border-b border-[#45a29e]/20 pb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Main infrastructure health and limits</p>
        </div>
      </div>
      ...
      {entries.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-400">No providers found. Run a benchmark first.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-[#45a29e]/10">
              {entries.map(([name, data]) => {
                ...
                return (
                  <tr key={name} className="hover:bg-[#45a29e]/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {bench.avgLatency ? `${Math.round(bench.avgLatency)}ms` : 'N/A'}
                    </td>
                    <td className="px-6 py-4">
                      {bench.avgTtfb ? `${Math.round(bench.avgTtfb)}ms` : 'N/A'}
                    </td>
                    <td className="px-6 py-4">
                      {bench.successRate !== undefined ? `${bench.successRate}%` : 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-right font-mono text-[#66fcf1]">
                      {typeof data.credits === 'number' ? `$${data.credits.toFixed(4)}` : data.credits}
                    </td>
                  </tr>
                )
              })}
