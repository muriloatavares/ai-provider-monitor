...
export default function ProviderAnalytics() {
  ...
  if (loading) return <div className="text-[#66fcf1] animate-pulse">Loading infrastructure analytics...</div>

  if (!providerData || !providerData.history || providerData.history.length === 0) {
    return (
      <div className="card text-center py-12">
        <Cpu className="w-12 h-12 mx-auto text-gray-600 mb-4" />
        <p className="text-gray-400">No underlying provider history available. Run benchmarks through OpenRouter first.</p>
      </div>
    )
  }
  ...
  const backends = Object.values(stats).map(s => {
  ...
  if (backends.length === 0) {
    return (
      <div className="card text-center py-12">
        <Cpu className="w-12 h-12 mx-auto text-gray-600 mb-4" />
        <p className="text-gray-400">OpenRouter did not return any 'x-openrouter-provider' headers in the recorded history.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 border-b border-[#45a29e]/20 pb-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-100">Infrastructure Analytics</h2>
          <p className="text-sm text-gray-400">Deep observability into OpenRouter's underlying compute clusters</p>
        </div>
      </div>
      ...
      <div className="card border border-[#66fcf1]/30 bg-[#1f2833]/80 mb-8 relative overflow-hidden">
        ...
        <h3 className="text-sm text-[#45a29e] font-bold uppercase tracking-widest mb-1">Top Performing Backend</h3>
        <h2 className="text-3xl font-black text-gray-100 mb-4">{bestBackend.name}</h2>
        <div className="flex gap-6">
          <div>
            <div className="text-xs text-gray-500">Success Rate</div>
            <div className="text-lg text-emerald-400 font-mono">{bestBackend.successRate.toFixed(1)}%</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Avg Latency</div>
            <div className="text-lg text-gray-200 font-mono">{Math.round(bestBackend.avgLatency)}ms</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Total Requests Handled</div>
            <div className="text-lg text-gray-200 font-mono">{bestBackend.requests}</div>
          </div>
        </div>
      </div>
              ...
              <div className="card p-5 hover:border-[#66fcf1]/30 transition-colors">
                <div className="flex justify-between items-start mb-4 border-b border-[#45a29e]/10 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-gray-100">{backend.name}</h3>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-[#66fcf1]">{backend.requests}</div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="bg-[#0b0c10]/50 p-2 rounded border border-[#1f2833]">
                    <div className="text-[10px] text-gray-500 uppercase flex items-center gap-1 mb-1"><Activity className="w-3 h-3"/> Latency</div>
                    <div className="font-mono text-gray-200">{Math.round(backend.avgLatency)}ms</div>
                  </div>
                  <div className="bg-[#0b0c10]/50 p-2 rounded border border-[#1f2833]">
                    <div className="text-[10px] text-gray-500 uppercase flex items-center gap-1 mb-1"><Zap className="w-3 h-3"/> TTFB</div>
                    <div className="font-mono text-gray-200">{Math.round(backend.avgTtfb)}ms</div>
                  </div>
                  <div className="bg-[#0b0c10]/50 p-2 rounded border border-[#1f2833]">
                    <div className="text-[10px] text-gray-500 uppercase flex items-center gap-1 mb-1"><ShieldCheck className="w-3 h-3 text-emerald-400"/> Success</div>
                    <div className="font-mono text-emerald-400">{backend.successRate.toFixed(1)}%</div>
                  </div>
                  <div className="bg-[#0b0c10]/50 p-2 rounded border border-[#1f2833]">
                    <div className="text-[10px] text-gray-500 uppercase flex items-center gap-1 mb-1"><ShieldCheck className="w-3 h-3 text-red-400"/> Failure</div>
                    <div className="font-mono text-red-400">{backend.failRate.toFixed(1)}%</div>
                  </div>
                </div>
              </div>
            </div>
