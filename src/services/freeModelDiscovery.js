...
class FreeModelDiscovery {
  ...
  async execute() {
    ...
    for (let i = 0; i < freeModelIds.length; i++) {
      ...
      logger.info(
        `Status: ${status.toUpperCase()} | Latency: ${bench.latency}ms`,
      );
      ...
      results.push({
        ...
        tps:
          bench.completionTokens && bench.latency
            ? parseFloat(
                (bench.completionTokens / (bench.latency / 1000)).toFixed(2),
              )
            : 0,
    ...
    const topOnline = finalReport.models
