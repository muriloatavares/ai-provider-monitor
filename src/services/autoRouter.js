...
class AutoRouter {
  generate() {
    ...
    if (!fs.existsSync(RANKING_FILE)) {
      logger.error('model_ranking.json not found! Run "npm run ranking" first.');
      process.exit(1);
    }
    ...
    let totalLatency = 0, totalTtfb = 0;
    ...
    logger.info(`Global Averages - Latency: ${Math.round(avgGlobalLatency)}ms | TTFB: ${Math.round(avgGlobalTtfb)}ms`);

    // Eligibility Criteria
    const eligibleModels = rankings.filter(r => {
      const isReliable = r.metrics.successRate >= 95 && r.reliabilityScore >= 90 && r.metrics.uptime >= 99;
      const isFast = r.metrics.avgLatency <= avgGlobalLatency && r.metrics.avgTTFB <= avgGlobalTtfb;
      return isReliable && isFast;
    });

    // If eligibility is too strict and returns empty, we relax the fast condition
    const pool = eligibleModels.length >= 3 ? eligibleModels : rankings.filter(r => r.metrics.successRate >= 90);

    // Generation of Chains (Top 3 for each category)
    const generateChain = (poolData, sortFn) => {
      return [...poolData].sort(sortFn).slice(0, 3).map(r => r.model);
    };

    const fastestChain = generateChain(pool, (a, b) => b.performanceScore - a.performanceScore);
    const cheapestChain = generateChain(pool, (a, b) => b.costScore - a.costScore);
    const reliableChain = generateChain(pool, (a, b) => b.reliabilityScore - a.reliabilityScore);
    const balancedChain = generateChain(pool, (a, b) => b.overallScore - a.overallScore);
