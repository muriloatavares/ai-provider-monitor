...
const extractKeys = (text) => {
  ...
  const foundKeys = [
    ...
    ...(text.match(anthropicRegex) || []).map(k => ({ key: k, provider: 'Anthropic' })),
    ...(text.match(geminiRegex) || []).map(k => ({ key: k, provider: 'Google AI' }))
  ];
...
const mapAxiosErrorToStatus = (err) => {
  if (err.response) {
    ...
    if (status === 401) return { keyStatus: 'invalid', errorMsg: 'Invalid Key (HTTP 401)' };
    if (status === 403) return { keyStatus: 'restricted', errorMsg: 'Permission Denied (HTTP 403)' };
    if (status === 429) return { keyStatus: 'rate_limited', errorMsg: 'Rate Limit Exceeded (HTTP 429)' };
    return { keyStatus: 'error', errorMsg: err.response?.data?.error?.message || `HTTP ${status}` };
  } else if (err.code === 'ECONNABORTED' || err.message.includes('timeout') || err.code === 'ENOTFOUND') {
...
const validateKey = async (item) => {
  try {
    if (item.provider === 'OpenRouter') {
      ...
      const response = await axios.get(`https://generativelanguage.googleapis.com/v1beta/models?key=${item.key}`);
