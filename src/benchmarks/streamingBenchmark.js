...
class StreamingBenchmark {
  ...
  async runStream(
    ...
    key,
    model,
    prompt = "Escreva um parágrafo sobre a história do JavaScript.",
  ) {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        response.data.on("end", () => {
          ...
          const tps =
            completionLength > 0 && streamDuration > 0
              ? completionLength / 4 / (streamDuration / 1000)
              : 0; // Estimation: 1 token = 4 chars
          const charactersPerSecond =
            streamDuration > 0 ? completionLength / (streamDuration / 1000) : 0;
          const avgChunkSize =
            chunksCount > 0 ? totalChunkSize / chunksCount : 0;
  ...
  async runAll() {
    ...
    for (const provider of activeProviders) {
      ...
      logger.info(
        `Chunks: ${result.totalChunks} (Avg Size: ${result.averageChunkSize} bytes)`,
      );
